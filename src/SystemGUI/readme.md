...
This directory, along with its subdirectories, contains the necessary files for developing components of the web interface for GATAS. If you're not working on the UI development, no additional steps are required—the C/C++ source files already integrate the web interface.
...
2. Connect the GATAS hardware to your local network by enabling the `WifiService` and confgure GATAS to connect to your network
3. In the `.proxyrc.js` file, update the `target` variable with the device’s IP address. GATAS will print the IP address over Serial line USB or Debug mode
