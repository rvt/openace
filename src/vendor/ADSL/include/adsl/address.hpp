...
namespace ADSL
{
    ...
    public:
        ...
        explicit Address(uint32_t asUintId)
            : Address(static_cast<uint8_t>(asUintId & 0xFF), static_cast<uint16_t>(asUintId >> 8 & 0xFFFF))
        {
        }
        ...
        {
            return (static_cast<uint32_t>(mfgId) & 0xFF) | (static_cast<uint32_t>(uniqueId) << 8);
        }
        ...
        static Address deserialize(etl::bit_stream_reader &reader)
        {
            ...
            address.uniqueId = reader.read_unchecked<uint16_t>();
            return address;
        }
    };
