...
/**
 * @brief A simple block allocator that allocates blocks from a fixed-size
 * memory pool.
 *
 * The goal is to have a fixed memory pool and allocate memory blocks
 * dynamically for different sizes ensuring that each allocated object maintains
 * a reference to its memory. The goal was to reduce memory for data blocks that
...
public:
   ...
   */
  typename etl::vector<T, MAX_BLOCKS>::iterator begin() {
    return allocatedBlocks.begin();
  }
   ...
   */
  typename etl::vector<T, MAX_BLOCKS>::iterator end() {
    return allocatedBlocks.end();
  }
   ...
   */
  typename etl::vector<T, MAX_BLOCKS>::const_iterator begin() const {
    return allocatedBlocks.begin();
  }
   ...
   */
  typename etl::vector<T, MAX_BLOCKS>::const_iterator end() const {
    return allocatedBlocks.end();
  }
   ...
   * @return A constant iterator to the beginning of the allocated blocks.
   */
  typename etl::vector<T, MAX_BLOCKS>::const_iterator cbegin() const {
    return allocatedBlocks.cbegin();
  }
   ...
   * @return A constant iterator to the end of the allocated blocks.
   */
  typename etl::vector<T, MAX_BLOCKS>::const_iterator cend() const {
    return allocatedBlocks.cend();
  }
