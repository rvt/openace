...
namespace ADSL
{
  static constexpr size_t SYNDROM_SIZE = 192;
  static constexpr uint32_t FindSyndrome192[SYNDROM_SIZE] = {
  ...
  static constexpr uint32_t Syndrome192[SYNDROM_SIZE] = {
  ...
  inline uint32_t CRCsyndrome(size_t Bit)
  {
    if (Bit >= SYNDROM_SIZE)
    {
      return 0;
    ...
    return Syndrome192[Bit];
  }

  inline uint8_t FindCRCsyndrome(uint32_t Syndr) // quick search for a single-bit CRC syndrome
  {
    for (size_t i = 0; i < SYNDROM_SIZE; ++i)
    {
      const uint32_t packed = FindSyndrome192[i];
      if ((packed >> 8) == Syndr)
      {
  ...
  {
    if (PktData.size() == 24 && PktErr.size() == 24)
    {
