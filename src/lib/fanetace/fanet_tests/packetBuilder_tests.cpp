...
TEST_CASE("PacketBuilder ACK Destination", "[single-file]")
{
    ...
    REQUIRE(pb.buildAck(0x987654) == make_etl_vector({0x80, 0x12, 0x56, 0x34, 0x20, 0x98, 0x54, 0x76}));
}
...
TEST_CASE("PacketBuilder ACK geo Forward", "[single-file]")
{
    PacketBuilder pb{0x123456};
    pb.isGeoForward();
    REQUIRE(pb.buildAck(0x987654) == make_etl_vector({0x80, 0x12, 0x56, 0x34, 0x21, 0x98, 0x54, 0x76,}));
}
...
TEST_CASE("PacketBuilder ACK isForward", "[single-file]")
{
    PacketBuilder pb{0x123456};
    pb.isForward();
    REQUIRE(pb.buildAck(0x987654) == make_etl_vector({0xC0, 0x12, 0x56, 0x34, 0x20, 0x98, 0x54, 0x76}));
}
...
TEST_CASE("PacketBuilder ACK signature", "[single-file]")
{
    ...
    pb.ack(ExtendedHeader::AckType::REQUEST_VIA_FORWARD);
    REQUIRE(pb.buildAck(0x987654) == make_etl_vector({0x80, 0x12, 0x56, 0x34, 0xB0, 0x98, 0x54, 0x76, 0x32, 0x54, 0x76, 0x98, }));
}
...
TEST_CASE("PacketBuilder Tracking Payload", "[single-file]")
{
    ...
    REQUIRE(pb.build(payload) == make_etl_vector({0x01, 0x12, 0x56, 0x34, 0xC0, 0x0E, 0x13, 0x00, 0x00, 0x00, 0x00, 0x00, 0x5B, 0x00, 0x00, 0x19 }));
}
...
TEST_CASE("PacketBuilder Name Payload", "[single-file]")
{
    ...
    PacketBuilder pb{Address(0x12, 0x3456)};
    REQUIRE(pb.build(payload) == make_etl_vector({0x02, 0x12, 0x56, 0x34, 0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x20, 0x57, 0x6F, 0x72, 0x6C, 0x64, }));
}
...
TEST_CASE("PacketBuilder Message Payload", "[single-file]")
{
    ...
    PacketBuilder pb{Address(0x12, 0x3456)};
    REQUIRE(pb.build(payload) == make_etl_vector({0x03, 0x12, 0x56, 0x34, 0x56, 0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x20, 0x57, 0x6F, 0x72, 0x6C, 0x64 }));
}
...
TEST_CASE("PacketBuilder GroundTracking Payload", "[single-file]")
{
    ...
    REQUIRE(pb.build(payload) == make_etl_vector({0x07, 0x12, 0x76, 0x98, 0x95, 0x8A, 0x4A, 0x81, 0x7F, 0xEE, 0x80, }));
}
