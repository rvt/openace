...
`StaticGPS` is a disabled-by-default, hardware-free `AbstractGnss` implementation. It reads decimal latitude, longitude, altitude above mean sea level, and an NTP server from configuration. Its own FreeRTOS software timer wakes a dedicated task every 500 ms; the task publishes checksum-valid GLL, RMC, VTG, GGA, and GSA sentences for a stationary aircraft tracking north.
