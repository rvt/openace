...
public:
    ...
    TcpClient(struct tcp_pcb *pcb)
        : ipPort(GATAS::Config::IpPort{pcb->remote_ip.addr, pcb->remote_port})
    {
        adopt(pcb);
    }
    ...
    void clearReceiveHandler() { onReceive = ReceiveHandler(); }
    void clearSentHandler() { onSent = SentHandler(); }
    void clearConnectHandler() { onConnect = ConnectHandler(); }
