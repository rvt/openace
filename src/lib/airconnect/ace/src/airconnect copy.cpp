...
void AirConnect::on_receive(const GATAS::DataPortMsg &msg)
{
    ...
    for (auto &it : connectedClients)
    {
        if (it.buffer.used() == 0)
        {
            it.buffer.setString(msg.sentence);
            // remove this from the buffer, but it ensures that if new data is added quickly before we receive the send msg, we have added 
            // this to the buffer, and don't send it directly ensuring correct flow control
            it.buffer.read(1);
        ...
        {
            if (!it.buffer.setString(msg.sentence)) {
...
err_t AirConnect::tcp_server_sent(void *arg, struct tcp_pcb *pcb, u16_t len)
{
    ...
    err_t err = ERR_OK;
    con_state->buffer.compact();

    auto oData = con_state->buffer.read(512);
    if (oData)
    {   
        auto data = oData.value();
        err = tcp_write(pcb, data.begin(), data.size(), TCP_WRITE_FLAG_COPY);
        if (data.size() > 400)
        {
            tcp_output(pcb);
        }
